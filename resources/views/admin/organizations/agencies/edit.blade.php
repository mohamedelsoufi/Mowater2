...
@section('title', __('words.edit_agency'))
@section('content')
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb {{app()->getLocale() == 'ar' ? 'float-sm-left' :  'float-sm-right'}}">
                            ...
                            <li class="breadcrumb-item active">{{__('words.edit_agency')}}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12">
                        <div class="card card-danger">
                            <div class="card-header">
                                <h3 class="card-title">{{__('words.edit_agency')}}</h3>
                            </div>
                            <form method="post"
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                ...
                                                <textarea name="description_ar"
                                                          class="form-control @error('description_ar') is-invalid @enderror">{{ old('description_ar',$agency->description_ar) }}</textarea>
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                ...
                                                <textarea name="description_en"
                                                          class="form-control @error('description_en') is-invalid @enderror">{{ old('description_en',$agency->description_en) }}</textarea>
                                                ...
                                                <select name="brand_id"
                                                    ...
                                                    @foreach($brands as $brand)
                                                        <option value="{{$brand->id}}"
                                                                @if (Input::old('brand_id') == $brand->id) selected @else {{$agency->brand_id == $brand->id ? 'selected' : ''}} @endif
                                                        >{{$brand->name}}</option>
                                                    @endforeach
                                                </select>
                                                @error('brand_id')
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <input type="text" name="year_founded"
                                                       ...
                                                       value="{{ old('year_founded',$agency->year_founded) }}"
                                            ...
                                            <div class="form-group col-md-4">
                                                ...
                                                <select name="country_id"
                                                    ...
                                                    @foreach($countries as $country)
                                                        ...
                                                        @else
                                                            <option
                                                                value="{{$country->id}}" {{$agency->country_id == $country->id ? "selected" : ""}}>{{$country->name}}</option>
                                                        @endif
                                                    @endforeach
                                                </select>
                                                @error('country_id')
                                            ...
                                            <div class="form-group col-md-3">
                                                <div class="form-check">
                                                    <input class="form-check-input" name="active" value="0"
                                                           {{$agency->active == 1 ? 'checked' : ''}} type="checkbox">
                                            ...
                                            <div class="form-group col-md-3">
                                                <div class="form-check">
                                                    <input class="form-check-input" name="active_number_of_views"
                                                           value="0"
                                                           {{$agency->active_number_of_views == 1 ? 'checked' : ''}} type="checkbox">
                                                    <label class="form-check-label">
                                            ...
                                            <div class="form-group col-md-3">
                                                <div class="form-check">
                                                    <input class="form-check-input" name="reservation_active" value="0"
                                                           {{$agency->reservation_active == 1 ? 'checked' : ''}} type="checkbox">
                                                    <label class="form-check-label">
                                            ...
                                            <div class="form-group col-md-3">
                                                <div class="form-check">
                                                    <input class="form-check-input" name="delivery_active" value="0"
                                                           {{$agency->delivery_active == 1 ? 'checked' : ''}} type="checkbox">
                                                    <label class="form-check-label">
...
@section('scripts')
    <script>
        $(document).ready(function () {
            get_cities("{{$agency->country_id}}", "{{$agency->city_id}}", "{{$agency->area_id}}");
            get_areas("{{$agency->city_id}}", "{{$agency->area_id}}");

            let url = "{{route('agency.users' , ':id')}}"
            url = url.replace(':id', "{{$agency->id}}");
            $.ajax({
                type: "Get",
                url: url,
                datatype: 'JSON',
        ...
        $('.email-change').on('change', function () {
            ...
            let get_user_url = "{{route('agency.user',[$agency->id,':user'])}}";
