...
@section('title', __('words.show_days_off'))
@section('content')
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb {{app()->getLocale() == 'ar' ? 'float-sm-left' :  'float-sm-right'}}">
                            ...
                            <li class="breadcrumb-item active">{{__('words.show_days_off')}}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">{{__('words.show_days_off')}}</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped text-center">
                                    ...
                                    @foreach($days_off as $key => $day_off)
                                        <tr>
                                            ...
                                            <td class="action">
                                                @if(auth('web')->user()->hasPermission(['read-day_offs-' . $record->name_en]))
                                                    <a href="{{route('organization.days-off.show',$day_off->id)}}"
                                                ...
                                                @if(auth('web')->user()->hasPermission(['delete-day_offs-' . $record->name_en]))
                                                    <a href="" class="btn btn-outline-danger"
                                                       data-toggle="modal"
                                                       data-target="#ModalDelete{{$day_off->id}}"
                                                       title="{{__('words.delete')}}">
                                                        <i class="fas fa-trash"></i>
                                                    </a>
                                                    @include('organization.daysOff.deleteModal')
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
