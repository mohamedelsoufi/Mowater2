...
@section('title', __('words.show_available_products'))
@section('content')
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb {{app()->getLocale() == 'ar' ? 'float-sm-left' :  'float-sm-right'}}">
                            ...
                            <li class="breadcrumb-item active">{{__('words.show_available_products')}}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">{{__('words.show_available_products')}}</h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="example1" class="table table-bordered table-striped text-center">
                                    ...
                                    @foreach($availableProducts as $key => $product)
                                        <tr>
                                            ...
                                            <td>{{$product->price}}</td>
                                            <td class="action">
                                                @if(auth('web')->user()->hasPermission(['read-available_products-' . $record->name_en]))
                                                    <a href="{{route('organization.available-products.show',$product->id)}}"
