                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb {{app()->getLocale() == 'ar' ? 'float-sm-left' :  'float-sm-right'}}">
                            ...
                            <li class="breadcrumb-item active">{{__('words.show_users')}}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                                ...
                                <table id="example1" class="table table-bordered table-striped text-center">
                                    ...
                                    @foreach($users as $key => $user)
                                        <tr>
                                            ...
                                            <td class="action">
                                                @if(auth('web')->user()->hasPermission('read-org_users-'. $record->name_en))
                                                    <a href="{{route('organization.users.show',$user->id)}}"
                                                ...
                                                @endif
                                                @if(auth('web')->user()->hasPermission('delete-org_users-'. $record->name_en))
                                                    ...
                                                    @include('organization.users.orgUsers.deleteModal')
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
