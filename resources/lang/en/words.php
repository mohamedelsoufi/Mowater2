<?php
return [
    // modules
    'mowater' => 'Mowater',
    'home' => 'Home',
    'admins' => 'admins',
    'brands' => 'Brands',
    'manufacture_country' => 'Manufacture Country',
    'manufacture_countries' => 'Manufacture Countries',
    'car_model' => 'Models',
    'car_class' => 'Car Classes',
    'countries' => 'Countries',
    'cities' => 'Cities',
    'areas' => 'Areas',
    'sections' => 'Sections',
    'org_sliders' => 'Organizations Sliders',
    'app_sliders' => 'App Sliders',
    'video_home_slider' => 'Video Slider',
    'home_first_slider' => 'First Slider',
    'home_second_slider' => 'Second Slider',
    'home_third_slider' => 'Third Slider',
    'payment_methods' => 'Payment Method',
    'colors' => 'Colors',
    'categories' => 'Categories',
    'special_numbers' => 'Special Numbers',
    'special_numbers_categories' => 'Special Numbers Categories',
    'special_numbers_organizations' => 'Special Numbers Organizations',
    'main_vehicles' => 'Vehicles',
    'agencies' => 'Agencies',
    'car_showrooms' => 'Cars Showroom',
    'rental_offices' => 'Rental Offices',
    'wenches' => 'Wenches',
    'AutoServiceCenters' => 'Auto Service Centers',
    'garages' => 'Garages',
    'scraps' => 'Scraps',
    'spare_parts' => 'Spare Parts',
    'brokers' => 'Brokers',
    'insurance_companies' => 'Insurance Companies',
    'delivery' => 'Delivery',
    'home_delivery' => 'Home Delivery',
    'distinctive_mark' => 'Distinctive Mark',
    'driving_trainers' => 'Driving Trainers',
    'discount_cards' => 'Mowater Cards',
    'sub_categories' => 'Sub Categories',
    'branches' => 'Branches',


    //nav bar
    'profile' => 'Profile',
    'inbox' => 'Inbox',
    'logout' => 'Logout',
    'search' => 'Search',

    //sidebar basic dropdown
    'show_all' => 'Show All',
    'vehicles_details' => 'Vehicles Details',
    'location' => 'Location',
    'general' => 'General',
    'organizations' => 'Organizations',
    'products' => 'Products',
    'services' => 'Services',
    'vehicles' => 'Vehicles',
    'reviews' => 'Reviews',
    'show_data' => 'Show Data',

    // auth
    'sign_in_your_account' => 'Sign in your account',
    'sign_in_your_account_org' => 'Sign in to your organization account',
    'email' => 'Email',
    'password' => 'Password',
    'confirm_password' => 'Confirm Password',
    'remember_me' => 'Remember me',
    'forgot_password' => 'Forgot Password ?',
    'sign_in' => 'Sign me in',
    'dont_have_account' => "Don't have an account ?",
    'sign_up' => "Sign up",
    'user_name' => 'User Name',

    // actions
    'actions' => 'Actions',
    'create' => 'Create',
    'update' => 'Update',
    'edit' => 'Edit',
    'show' => 'Show',
    'delete' => 'Delete',
    'close' => 'Close',
    'choose' => 'Choose...',
    'choose_image' => 'Choose Image',
    'choose_images' => 'Choose Images',
    'choose_file' => 'Choose File',
    'save' => 'Save',

    // attributes
    'active' => 'Active',
    'inactive' => 'Inactive',
    'available_prop' => 'Available',
    'not_available_prop' => 'Not Available',
    'busy' => 'Busy',
    'name_en' => 'Name In English',
    'name_ar' => 'Name in Arabic',
    'name' => 'Name',
    'description' => 'Description',
    'activity' => 'Activity Status',
    'org_activity' => 'Organization Activity Status',
    'country' => 'Country',
    'features' => 'Features',
    'city' => 'City',
    'area' => 'Area',
    'brand' => 'Brand',
    'section' => 'Section',
    'ref_name' => 'Reference Name',
    'reservation_cost_type' => 'Reservation Cost Type',
    'reservation_cost' => 'Reservation Cost',
    'new_brand' => 'Create New Brand',
    'show_brand' => 'Show Brand',
    'show_brands' => 'Show Brands',
    'edit_brand' => 'Edit Brand',
    'new_manufacture_country' => 'Create Manufacture Country',
    'show_manufacture_country' => 'Show Manufacture Country',
    'show_manufacture_countries' => 'Show Manufacture Countries',
    'edit_manufacture_country' => 'Edit Manufacture Countries',
    'new_car_model' => 'Create New Model',
    'show_car_model' => 'Show Model',
    'show_car_models' => 'Show Models',
    'edit_car_model' => 'Edit Model',
    'new_car_class' => 'Create Class',
    'show_car_class' => 'Show Class',
    'show_car_classes' => 'Show Classes',
    'edit_car_class' => 'Edit Class',
    'new_country' => 'Create Country',
    'show_country' => 'Show Country',
    'show_countries' => 'Show Countries',
    'edit_country' => 'Edit Country',
    'new_city' => 'Create City',
    'show_city' => 'Show City',
    'show_cities' => 'Show Cities',
    'edit_city' => 'Edit City',
    'show_areas' => 'Show Areas',
    'edit_area' => 'Edit Area',
    'new_area' => 'Create Area',
    'show_area' => 'Show Area',
    'show_section' => 'Show Section',
    'show_sections' => 'Show Sections',
    'show_sub_sections' => 'Show Sub Sections',
    'edit_section' => 'Edit Section',
    'sub_section' => 'Sub Section',
    'none' => 'None',
    'new_slider' => 'Create Slider',
    'show_slider' => 'Show Slider',
    'show_sliders' => 'Show Sliders',
    'edit_sliders' => 'Edit Slider',
    'image' => 'Image \ Video',
    'vehicle_image' => 'Vehicle Image',
    'type' => 'type',
    'app_home' => 'Application Home Page',
    'app_home_image' => 'Application Home Page Image',
    'app_home_video' => 'Application Home Page Video',
    'for_section' => 'For Section',
    'Slider_image_Video' => 'Slider image / Video',
    'new_payment_method' => 'Create Payment Method',
    'show_payment_method' => 'Show Payment Method',
    'show_payment_methods' => 'Show Payment Methods',
    'edit_payment_method' => 'Edit Payment Method',
    'new_color' => 'Create Color',
    'show_color' => 'Show Color',
    'show_colors' => 'Show Colors',
    'edit_color' => 'Edit Color',
    'color_name' => 'Color Name in English',
    'color_name_ar' => 'Color Name in Arabic',
    'color_code' => 'Color Code',
    'new_category' => 'Create Category',
    'show_category' => 'Show Category',
    'show_categories' => 'Show Categories',
    'edit_category' => 'Edit Category',
    'new_special_number_category' => 'Create Special Number Category',
    'show_special_number_category' => 'Show Special Number Category',
    'new_special_number_organization' => 'Create Special Number Organization',
    'show_special_number_organization' => 'Show Special Number Organization',
    'show_special_number_organizations' => 'Show Special Number Organizations',
    'edit_special_number_organization' => 'Edit Special Number Organization',
    'description_en' => 'Description In English',
    'description_ar' => 'Description In Arabic',
    'tax_number' => 'Tax umber',
    'logo' => 'Logo',
    'available' => 'Availability',
    'reservation_availability' => 'Reservation Availability',
    'delivery_availability' => 'Delivery Availability',
    'reservation_active' => 'Reservation Activity',
    'delivery_active' => 'Delivery Activity',
    'year_founded' => 'Year Founded',
    'images' => 'Images',
    'new_main_vehicle' => 'Create Main Vehicle',
    'show_main_vehicle' => 'Show Main Vehicle',
    'new_agency' => 'Create Agency',
    'show_agency' => 'Show Agency',
    'show_agencies' => 'Show Agencies',
    'edit_agency' => 'Edit Agency',
    'new_car_showrooms' => 'Create Car showroom',
    'show_car_showroom' => 'Show Car Showroom',
    'show_car_showrooms' => 'Show Car Showrooms',
    'edit_car_showroom' => 'Edit Car Showrooms',
    'new_rental_office' => 'Create Rental Office',
    'show_rental_office' => 'Show Rental Office',
    'show_rental_offices' => 'Show Rental Offices',
    'edit_rental_office' => 'Edit Rental Office',
    'new_wench' => 'Create Wench',
    'show_wench' => 'Show Wench',
    'show_wenches' => 'Show Wenches',
    'edit_wench' => 'Edit Wench',
    'wench_type' => 'Wench Type',
    'new_garage' => 'Create Garages',
    'show_garage' => 'Show Garage',
    'show_garages' => 'Show Garages',
    'edit_garage' => 'Edit Garage',
    'new_special_number' => 'Create Special Number',
    'show_special_number' => 'Show Special Number',
    'edit_special_number' => 'Edit Special Number',
    'show_special_numbers' => 'Show Special Numbers',
    'special_numbers_category' => 'Special Numbers Category',
    'number' => 'Number',
    'size' => 'Size',
    'transfer_type' => 'Transfer Type',
    'price' => 'Price',
    'total_price' => 'Total Price',
    'Include_insurance' => 'Include Insurance',
    'user' => 'User Name',
    'special_numbers_organization' => 'Special Numbers Organization',
    'waiver' => 'Waiver',
    'own' => 'Own',
    'including' => 'Including',
    'excl' => 'Excl',
    'new_scrap' => 'Create Scrap',
    'show_scrap' => 'Show Scrap',

    'new_spare_part' => 'Create Spare Part',
    'show_spare_part' => 'Show Spare Part',

    'new_broker' => 'Create Broker',
    'show_broker' => 'Show Broker',
    'show_brokers' => 'Show Brokers',
    'edit_broker' => 'Edit Broker',

    'new_insurance_company' => 'Create Insurance Company',
    'show_insurance_company' => 'Show Insurance Company',
    'show_insurance_companies' => 'Show Insurance Companies',
    'edit_insurance_company' => 'Edit Insurance Company',

    'new_delivery' => 'Create Delivery Man',
    'show_delivery' => 'Show Delivery Man',
    'show_deliveries' => 'Show Delivery Men',
    'edit_delivery' => 'Edit Delivery Man',
    'gender' => 'Gender',
    'birth_date' => 'Birth Date',
    'conveyor_type' => 'Conveyor Type',
    'age' => 'Age',
    'delivery_type' => 'Delivery Type',

    'new_driving_trainer' => 'Create Driving Trainer',
    'show_driving_trainer' => 'Show Driving Trainer',
    'show_driving_trainers' => 'Show Driving Trainers',
    'edit_driving_trainer' => 'Edit Driving Trainer',
    'record_number' => 'record number',
    'requirements_ar' => 'Requirements in Arabic',
    'requirements_en' => 'Requirements in English',


    'show_org_data' => 'Show data for ',
    'edit_org_data' => 'Edit data for ',
    'vehicle_status' => 'Vehicle Status',
    'availability' => 'Availability',
    'bhd' => 'BHD',


    //products
    'product_status' => 'product status',
    'product_warranty' => 'Warranty',
    'product_is_new' => 'new or used',


    'required' => 'required',
    'not_required' => 'not required',
    'location_required' => 'location is required',
    'show_products' => 'Show Products',
    'show_product' => 'Show Product',
    'edit_product' => 'Edit Product',
    'new_product' => 'Create Product',
    'show_services' => 'Show Services',
    'show_service' => 'Show Service',
    'edit_service' => 'Edit Service',
    'create_service' => 'Create Service',
    //ads
    'show_ad' => 'show ad',
    'show_ads' => 'show ads',
    'edit_ad' => 'Edit ad',
    'create_ad' => 'Create ad',
    'external_ad_link' => 'External Ad Link',
    'ad_in_app' => 'Ad In App',
    'ads' => 'Ads',
    'negotiable' => 'Negotiable ',
    'accepted' => 'accepted',
    'not_accepted' => 'not accepted',
    'title_en' => 'English title',
    'title_ar' => 'Arabic title',
    'new_ad' => 'new ad',
    'title' => 'Title',


    'show_vehicles' => 'Show Vehicles',
    'show_rental_cars' => 'Show Rental Office Cars',
    'new_vehicle' => 'Create Vehicle',
    'new_rental_car' => 'Create Car',
    'show_vehicle' => 'Show Vehicle',
    'show_rental_car' => 'Show Rental Office Car',
    'select_vehicle' => 'Select Vehicle',
    'vehicle_car_model' => 'Model',
    'vehicle_car_class' => 'Class',
    'january' => 'January',
    'february' => 'February',
    'march' => 'March',
    'april' => 'April',
    'may' => 'May',
    'june' => 'June',
    'july' => 'July',
    'august' => 'August',
    'september' => 'September',
    'october' => 'October',
    'november' => 'November',
    'december' => 'December',
    'add_more_images' => 'Add more images',
    'traveled_distance' => 'Traveled Distance',
    'traveled_distance_type' => 'Traveled Distance Type',
    'car_status' => 'Car Status',
    'guarantee' => 'Guarantee',
    'certified_maintenance' => 'Certified Maintenance',
    'selling_by_plate' => 'Selling By Plate field is required ',
    'number_plate' => 'Selling By Plate',
    'price_is_negotiable' => 'Price Is Negotiable',
    'in_bahrain' => 'In Bahrain',
    'back' => 'Back',
    'product_car_model' => 'The suitable car models and its manufacturing years',

    'area_s' => 'Area',
    'year' => 'Year',
    'month' => 'Month',
    'additional_notes' => 'Additional Notes',
    's_car_model' => 'Car Model',
    's_car_class' => 'Car Class',
    'not_mentioned' => 'not mentioned',
    'reservations' => 'Reservations',

    'special_number_reservations' => 'Special Number Reservations',
    'special_number' => 'Special Number',
    'user_email' => 'User E-mail',
    'phone' => 'Phone',
    'phone_title' => 'Short address of the phone number',
    'address' => 'Address',
    'status' => 'Status',
    'reservation_time' => 'Reservation Time',
    'show_reservation' => 'Show Reservation',
    'show_reservations' => 'Show Reservations',
    'edit_reservation' => 'Edit Reservation',
    'special_number_data' => 'Special Number Data',
    'user_data' => 'User Data',

    'verifications' => 'Verifications',
    'show_verifications' => 'Show Verifications',
    'normal_plate' => 'Normal Plate',
    'special_plate' => 'Special Plate',

    'main_category' => 'Main Category',
    'private_car_numbers' => 'Private car numbers',
    'shared_car_numbers' => 'Shared vehicle numbers (pickup)',
    'motorcycle_numbers' => 'Motorcycle numbers',

    'nationality' => 'Nationality',
    'bahraini' => 'Bahraini',
    'foreign' => 'Foreign',

    'price_include_transfer' => 'The price includes transfer fees',
    'image_s' => 'Image',
    'currencies' => 'Currencies',
    'currency' => 'Currency',
    'new_currency' => 'Create Currency',
    'show_currency' => 'Show Currency',
    'show_currencies' => 'Show Currencies',
    'edit_currency' => 'Edit Currency',
    'currency_code' => 'Currency Code',
    'currency_name_en' => 'Currency Name in En',
    'currency_name_ar' => 'Currency Name in Ar',

    'delivery_men' => 'Delivery Men',
    'old_license' => 'Has old license',

    'test_reservation_availability' => 'Reservation Availability For Test Drive',
    'delivery_count' => 'Count',

    'new_discount_card' => 'Create Mowater Card',
    'show_discount_card' => 'Show Mowater Card',
    'show_discount_card_orgs' => 'Show Mowater Card Organizations',
    'show_discount_card_org' => 'Show Mowater Card Organization',
    'edit_discount_card_org' => 'Edit Mowater Card Organization',
    'show_discount_cards' => 'Show Mowater Cards',
    'edit_discount_card' => 'Edit Mowater Card',
    'not_started' => 'Not Launched',
    'started' => 'Launch',
    'finished' => 'Finished',
    'subscribe' => 'Subscribe',
    'discount_card_offers' => 'Mowater Card Offer',
    'select' => 'Select',
    'discount_type' => 'Discount Type',
    'value' => 'Value',
    'percentage' => 'Percentage',
    'amount' => 'Amount',
    'discount_value' => 'Discount Value',
    'discount_card_offer' => 'Mowater Card Offer',
    'show_offers' => 'Offer',

    'one_time' => 'One Time',
    'endless' => 'Endless',
    'specific_number' => 'Let it blank if it be endless',
    'number_of_uses_times' => 'Number Of Uses Times',

    'auctions' => 'Auctions',
    'new_auction' => 'Add new auction',
    'edit_auction' => 'Edit an auction',
    'end_date_time' => 'End date and time',
    'start_date_time' => 'Start date and time',
    'insurance_amount' => 'Insurance amount',
    'Product' => 'product',
    'Service' => 'service',
    'SpecialNumber' => 'Special number',
    'start_price' => 'start price',
    'bids' => 'Bids',
    'bid_date' => 'Bid date',
    'ended' => 'Ended',
    'already_started' => 'Already started',

    'threeDFile' => '3D File',
    'add_more_3d' => 'Add more 3D File',

    'count_of_numbers' => 'Count of numbers',
    'numbers' => 'Numbers',
    'ref_name_category' => 'Category reference name',

    'sub_category' => 'Sub category',
    'new_sub_category' => 'Create sub category',
    'show_sub_category' => 'Show sub category',
    'show_sub_categories' => 'Show sub categories',
    'edit_sub_category' => 'Edit sub category',
    'good' => 'Good',
    'super' => 'super',
    'very_good' => 'Very good',
    'excellent' => 'Excellent',
    'law' => 'Law',
    'law_ar' => 'Law In Arabic',
    'law_en' => 'Law In English',
    'latitude' => 'Latitude',
    'longitude' => 'longitude',
    'click_here_to_get_your_coordinates' => 'Click here to get your coordinates.',
    'users' => 'Users',
    'organization' => 'Organization',
    'organization_name' => 'Organization Name',
    'new_user' => 'Create User',
    'edit_user' => 'Edit User',

    'fuel_stations' => 'Fuel Stations',
    'new_fuel_station' => 'Create fuel station',
    'show_fuel_station' => 'Show fuel station',
    'show_fuel_stations' => 'Show fuel stations',
    'edit_fuel_station' => 'Edit fuel station',
    'fuel_types' => 'Fuel types',
    'Gasoline_91' => 'Gasoline 91',
    'Gasoline_95' => 'Gasoline 95',
    'Gasoline_98' => 'Gasoline 98',
    'Diesel' => 'Diesel',

    'contacts' => 'Contacts',
    'show_contacts' => 'Show Contacts',
    'edit_contacts' => 'Edit Contacts',
    'facebook_link' => 'Facebook linf',
    'whatsapp_number' => 'Whats app',
    'country_code' => 'Country code',
    'website' => 'Website',
    'instagram_link' => 'Instagram ling',

    // countries
    'Other_countries' => 'Other countries',
    'Bahrain' => 'Bahrain',
    'Algeria' => 'Algeria',
    'Andorra' => 'Andorra',
    'Angola' => 'Angola',
    'Anguilla' => 'Anguilla',
    'Antigua&Barbuda' => 'Antigua & Barbuda',
    'Argentina' => 'Argentina',
    'Armenia' => 'Armenia',
    'Aruba' => 'Aruba',
    'Australia' => 'Australia',
    'Austria' => 'Austria',
    'Azerbaijan' => 'Azerbaijan',
    'Bahamas' => 'Bahamas',
    'Bangladesh' => 'Bangladesh',
    'Barbados' => 'Barbados',
    'Belarus' => 'Belarus',
    'Belgium' => 'Belgium',
    'Belize' => 'Belize',
    'Benin' => 'Benin',
    'Bermuda' => 'Bermuda',
    'Bhutan' => 'Bhutan',
    'Bolivia' => 'Bolivia',
    'Bosnia Herzegovina' => 'Bosnia Herzegovina',
    'Botswana' => 'Botswana',
    'Brazil' => 'Brazil',
    'Brunei' => 'Brunei',
    'Bulgaria' => 'Bulgaria',
    'Burkina Faso' => 'Burkina Faso',
    'Burundi' => 'Burundi',
    'Cambodia' => 'Cambodia',
    'Cameroon' => 'Cameroon',
    'Canada' => 'Canada',
    'Cape Verde Islands' => 'Cape Verde Islands',
    'Cayman Islands' => 'Cayman Islands',
    'Central African Republic' => 'Central African Republic',
    'Chile' => 'Chile',
    'China' => 'China',
    'Colombia' => 'Colombia',
    'Comoros' => 'Comoros',
    'Congo' => 'Congo',
    'Cook Islands' => 'Cook Islands',
    'Costa Rica' => 'Costa Rica',
    'Croatia' => 'Croatia',
    'Cuba' => 'Cuba',
    'Cyprus North' => 'Cyprus North',
    'Cyprus South' => 'Cyprus South',
    'Czech Republic' => 'Czech Republic',
    'Denmark' => 'Denmark',
    'Djibouti' => 'Djibouti',
    'Dominica' => 'Dominica',
    'Dominican Republic' => 'Dominican Republic',
    'Ecuador' => 'Ecuador',
    'Egypt' => 'Egypt',
    'El Salvador' => 'El Salvador',
    'Equatorial Guinea' => 'Equatorial Guinea',
    'Eritrea' => 'Eritrea',
    'Estonia' => 'Estonia',
    'Ethiopia' => 'Ethiopia',
    'Falkland Islands' => 'Falkland Islands',
    'Faroe Islands' => 'Faroe Islands',
    'Fiji' => 'Fiji',
    'Finland' => 'Finland',
    'France' => 'France',
    'French Guiana' => 'French Guiana',
    'French Polynesia' => 'French Polynesia',
    'Gabon' => 'Gabon',
    'Gambia' => 'Gambia',
    'Georgia' => 'Georgia',
    'Germany' => 'Germany',
    'Ghana' => 'Ghana',
    'Gibraltar' => 'Gibraltar',
    'Greece' => 'Greece',
    'Greenland' => 'Greenland',
    'Grenada' => 'Grenada',
    'Guadeloupe' => 'Guadeloupe',
    'Guam' => 'Guam',
    'Guatemala' => 'Guatemala',
    'Guinea' => 'Guinea',
    'Guinea - Bissau' => 'Guinea - Bissau',
    'Guyana' => 'Guyana',
    'Haiti' => 'Haiti',
    'Honduras' => 'Honduras',
    'Hong Kong' => 'Hong Kong',
    'Hungary' => 'Hungary',
    'Iceland' => 'Iceland',
    'India' => 'India',
    'Indonesia' => 'Indonesia',
    'Iran' => 'Iran',
    'Iraq' => 'Iraq',
    'Ireland' => 'Ireland',
    'Israel' => 'Israel',
    'Italy' => 'Italy',
    'Jamaica' => 'Jamaica',
    'Japan' => 'Japan',
    'Jordan' => 'Jordan',
    'Kazakhstan' => 'Kazakhstan',
    'Kenya' => 'Kenya',
    'Kiribati' => 'Kiribati',
    'Korea North' => 'Korea North',
    'Korea South' => 'Korea South',
    'Kuwait' => 'Kuwait',
    'Kyrgyzstan' => 'Kyrgyzstan',
    'Laos' => 'Laos',
    'Latvia' => 'Latvia',
    'Lebanon' => 'Lebanon',
    'Lesotho' => 'Lesotho',
    'Liberia' => 'Liberia',
    'Libya' => 'Libya',
    'Liechtenstein' => 'Liechtenstein',
    'Lithuania' => 'Lithuania',
    'Luxembourg' => 'Luxembourg',
    'Macao' => 'Macao',
    'Macedonia' => 'Macedonia',
    'Madagascar' => 'Madagascar',
    'Malawi' => 'Malawi',
    'Malaysia' => 'Malaysia',
    'Maldives' => 'Maldives',
    'Mali' => 'Mali',
    'Malta' => 'Malta',
    'Marshall Islands' => 'Marshall Islands',
    'Martinique' => 'Martinique',
    'Mauritania' => 'Mauritania',
    'Mayotte' => 'Mayotte',
    'Mexico' => 'Mexico',
    'Micronesia' => 'Micronesia',
    'Moldova' => 'Moldova',
    'Monaco' => 'Monaco',
    'Mongolia' => 'Mongolia',
    'Montserrat' => 'Montserrat',
    'Morocco' => 'Morocco',
    'Mozambique' => 'Mozambique',
    'Myanmar' => 'Myanmar',
    'Namibia' => 'Namibia',
    'Nauru' => 'Nauru',
    'Nepal' => 'Nepal',
    'Netherlands' => 'Netherlands',
    'New Caledonia' => 'New Caledonia',
    'New Zealand' => 'New Zealand',
    'Nicaragua' => 'Nicaragua',
    'Niger' => 'Niger',
    'Nigeria' => 'Nigeria',
    'Niue' => 'Niue',
    'Norfolk Islands' => 'Norfolk Islands',
    'Northern Marianas' => 'Northern Marianas',
    'Norway' => 'Norway',
    'Oman' => 'Oman',
    'Palau' => 'Palau',
    'Panama' => 'Panama',
    'Papua New Guinea' => 'Papua New Guinea',
    'Paraguay' => 'Paraguay',
    'Peru' => 'Peru',
    'Philippines' => 'Philippines',
    'Poland' => 'Poland',
    'Portugal' => 'Portugal',
    'Puerto Rico' => 'Puerto Rico',
    'Qatar' => 'Qatar',
    'Reunion' => 'Reunion',
    'Romania' => 'Romania',
    'Russia' => 'Russia',
    'Rwanda' => 'Rwanda',
    'San Marino' => 'San Marino',
    'Sao Tome & Principe' => 'Sao Tome & Principe',
    'Saudi Arabia' => 'Saudi Arabia',
    'Senegal' => 'Senegal',
    'Serbia' => 'Serbia',
    'Seychelles' => 'Seychelles',
    'Sierra Leone' => 'Sierra Leone',
    'Singapore' => 'Singapore',
    'Slovak Republic' => 'Slovak Republic',
    'Slovenia' => 'Slovenia',
    'Solomon Islands' => 'Solomon Islands',
    'Somalia' => 'Somalia',
    'South Africa' => 'South Africa',
    'Spain' => 'Spain',
    'Sri Lanka' => 'Sri Lanka',
    'St. Helena' => 'St. Helena',
    'St. Kitts' => 'St. Kitts',
    'St. Lucia' => 'St. Lucia',
    'Sudan' => 'Sudan',
    'Suriname' => 'Suriname',
    'Swaziland' => 'Swaziland',
    'Sweden' => 'Sweden',
    'Switzerland' => 'Switzerland',
    'Syria' => 'Syria',
    'Taiwan' => 'Taiwan',
    'Tajikstan' => 'Tajikstan',
    'Thailand' => 'Thailand',
    'Togo' => 'Togo',
    'Tonga' => 'Tonga',
    'Trinidad & Tobago' => 'Trinidad & Tobago',
    'Tunisia' => 'Tunisia',
    'Turkey' => 'Turkey',
    'Turkmenistan' => 'Turkmenistan',
    'Turks & Caicos Islands' => 'Turks & Caicos Islands',
    'Tuvalu' => 'Tuvalu',
    'Uganda' => 'Uganda',
    'UK' => 'UK',
    'Ukraine' => 'Ukraine',
    'United Arab Emirates' => 'United Arab Emirates',
    'Uruguay' => 'Uruguay',
    'USA' => 'USA',
    'Uzbekistan' => 'Uzbekistan',
    'Vanuatu' => 'Vanuatu',
    'Vatican' => 'Vatican',
    'Venezuela' => 'Venezuela',
    'Vietnam' => 'Vietnam',
    'Virgin Islands - British' => 'Virgin Islands - British',
    'Virgin Islands - US' => 'Virgin Islands - US',
    'Wallis & Futuna' => 'Wallis & Futuna',
    'Yemen (North)' => 'Yemen (North)',
    'Yemen (South)' => 'Yemen (South)',
    'Zambia' => 'Zambia',
    'Zimbabwe' => 'Zimbabwe',

    'verified' => 'You have successfully verified your email address.!',
    'invalid-verification' => 'Verification code is invalid.',
    'discount_availability' => 'Discount Availability',


    'Vehicle_inspection' => 'Vehicle inspection',
    'Pay_fines' => 'Pay fines',
    'Vehicle_registration_or_renewal' => 'Vehicle registration or renewal',
    'Vehicle_insurance' => 'Vehicle insurance',
    'Transfer_of_vehicle_ownership' => 'Transfer of vehicle ownership',
    'Changing_the_plates' => 'Changing the plates',

    'renewal' => 'Renewal',
    'transfer_from_one_company_to_another' => 'Transfer from one company to another',
    'transfer_to_another_person' => 'Transfer to another person',
    'first_once' => 'First once',

    //dashboard
    'dashboard' => 'Dashboard',
    'admin_dashboard' => 'Admin Dashboard',
    'organization_dashboard' => 'Organization Dashboard',
    'branch_dashboard' => 'Branch Dashboard',
    'admin_sig_in' => 'Admin Sign in',
    'org_sig_in' => 'Organization Sign in',
    'app_users' => 'App Users',
    'admin_users' => 'Admin Users',
    'orgs_users' => 'Organizations Users',
    'org_users' => 'Organization Users',
    'branches_users' => 'Branches Users',
    'branch_users' => 'Branch Users',
    'show_users' => 'Show Users',
    'show_user' => 'Show User',
    'create_admin_user' => 'Create Admin User',
    'edit_admin_user' => 'Edit Admin User',
    'show_admin_users' => 'Show Admin Users',
    'show_admin_user' => 'Show Admin User',
    'copy' => 'Copy',
    'print' => 'Print',
    'pdf' => 'PDF',
    'excel' => 'Excel',
    'csv' => 'CSV',
    'column_visibility' => 'Column Visibility',
    'loading' => 'Please wait - loading...',
    'showing' => 'Showing',
    'entries' => 'entries',
    'of' => 'of',
    'to' => 'to',
    'nickname' => 'Nickname',
    'first_name' => 'First Name',
    'last_name' => 'Last Name',
    'phone_code' => 'Phone Code',
    'delete_confirm' => 'Delete Confirmation',
    'cancel' => 'Cancel',

    'super_admin' => 'Super Admin',
    'read' => 'read',
    'permissions' => 'Permissions',
    'display_name' => 'Display Name',

    'roles' => 'Roles',
    'show_roles' => 'Show Roles',
    'show_role' => 'Show Role',
    'create_role' => 'Create Role',
    'edit_role' => 'Edit Role',
    'car_models' => 'Car Models',
    'car_classes' => 'Car Classes',
    'number_of_views' => 'Number of views',
    'active_number_of_views' => 'Activate number of views',
    'is_special' => 'Special',
    'price_after_discount' => 'Price after discount',
    'coverage_range' => 'Coverage range',
    'local' => 'Local',
    'national' => 'National',

    'traffic_clearing_offices' => 'Traffic Clearing Offices',
    'show_traffic_clearing_offices' => 'Show Traffic Clearing Offices',
    'show_traffic_clearing_office' => 'Show Traffic Clearing Office',
    'edit_traffic_clearing_office' => 'Edit Traffic Clearing Office',
    'new_traffic_clearing_office' => 'Create Traffic Clearing Office',

    'more_info' => 'More info',

    'technical_inspection_centers' => 'Technical Inspection Centers',
    'show_technical_inspection_center' => 'Show Technical Inspection Center',
    'show_technical_inspection_centers' => 'Show Technical Inspection Centers',
    'edit_technical_inspection_center' => 'Edit Technical Inspection Center',
    'new_technical_inspection_center' => 'Create Technical Inspection Center',

    'tire_exchange_centers' => 'Tire Exchange Centers',
    'show_tire_exchange_center' => 'Show Tire Exchange Center',
    'show_tire_exchange_centers' => 'Show Tire Exchange Centers',
    'edit_tire_exchange_center' => 'Edit Tire Exchange Center',
    'new_tire_exchange_center' => 'Create Tire Exchange Center',

    'accessories_stores' => 'Accessories Stores',
    'show_accessories_store' => 'Show Accessories Store',
    'show_accessories_stores' => 'Show Accessories Stores',
    'edit_accessories_store' => 'Edit Accessories Store',
    'new_accessories_store' => 'Create Accessories Store',

    'car_washes' => 'Car Wash, polishing and service',
    'show_car_wash' => 'Show Car Wash',
    'show_car_washes' => 'Show Car Washes',
    'edit_car_wash' => 'Edit Car Wash',
    'new_car_wash' => 'Create Car Wash',

    'mining_centers' => 'Mining centers and energy providers',
    'show_mining_center' => 'Show Mining center',
    'show_mining_centers' => 'Show Mining centers',
    'edit_mining_center' => 'Edit Mining center',
    'new_mining_center' => 'Create Mining center',

    'created_at' => 'Created at',
    'created_by' => 'Created by',
    'updated_at' => 'Updated at',

    'verification' => 'Verification',
    'is_verified' => 'Verified',
    'not_verified' => 'Not Verified',

    'show_app_users' => 'Show App Users',
    'show_app_user' => 'Show App User',
    'edit_app_user' => 'Edit App User',

    'show_org_users' => 'Show Organization Users',
    'show_org_user' => 'Show Organization User',
    'edit_org_user' => 'Edit Organization User',
    'new_org_user' => 'Create Organization User',

    //country codes
    "Andorra (+376)" => "Andorra (+376)",
    "Angola (+244)" => "Angola (+244)",
    "Anguilla (+1264)" => "Anguilla (+1264)",
    "Antigua &amp; Barbuda (+1268)" => "Antigua &amp; Barbuda (+1268)",
    "Argentina (+54)" => "Argentina (+54)",
    "Armenia (+374)" => "Armenia (+374)",
    "Aruba (+297)" => "Aruba (+297)",
    "Australia (+61)" => "Australia (+61)",
    "Austria (+43)" => "Austria (+43)",
    "Azerbaijan (+994)" => "Azerbaijan (+994)",
    "Bahamas (+1242)" => "Bahamas (+1242)",
    "Bahrain (+973)" => "Bahrain (+973)",
    "Bangladesh (+880)" => "Bangladesh (+880)",
    "Barbados (+1246)" => "Barbados (+1246)",
    "Belarus (+375)" => "Belarus (+375)",
    "Belgium (+32)" => "Belgium (+32)",
    "Belize (+501)" => "Belize (+501)",
    "Benin (+229)" => "Benin (+229)",
    "Bermuda (+1441)" => "Bermuda (+1441)",
    "Bhutan (+975)" => "Bhutan (+975)",
    "Bolivia (+591)" => "Bolivia (+591)",
    "Bosnia Herzegovina (+387)" => "Bosnia Herzegovina (+387)",
    "Botswana (+267)" => "Botswana (+267)",
    "Brazil (+55)" => "Brazil (+55)",
    "Brunei (+673)" => "Brunei (+673)",
    "Bulgaria (+359)" => "Bulgaria (+359)",
    "Burkina Faso (+226)" => "Burkina Faso (+226)",
    "Burundi (+257)" => "Burundi (+257)",
    "Cambodia (+855)" => "Cambodia (+855)",
    "Cameroon (+237)" => "Cameroon (+237)",
    "Canada (+1)" => "Canada (+1)",
    "Cape Verde Islands (+238)" => "Cape Verde Islands (+238)",
    "Cayman Islands (+1345)" => "Cayman Islands (+1345)",
    "Central African Republic (+236)" => "Central African Republic (+236)",
    "Chile (+56)" => "Chile (+56)",
    "China (+86)" => "China (+86)",
    "Colombia (+57)" => "Colombia (+57)",
    "Comoros (+269)" => "Comoros (+269)",
    "Congo (+242)" => "Congo (+242)",
    "Cook Islands (+682)" => "Cook Islands (+682)",
    "Costa Rica (+506)" => "Costa Rica (+506)",
    "Croatia (+385)" => "Croatia (+385)",
    "Cuba (+53)" => "Cuba (+53)",
    "Cyprus North (+90392)" => "Cyprus North (+90392)",
    "Cyprus South (+357)" => "Cyprus South (+357)",
    "Czech Republic (+42)" => "Czech Republic (+42)",
    "Denmark (+45)" => "Denmark (+45)",
    "Djibouti (+253)" => "Djibouti (+253)",
    "Dominica (+1809)" => "Dominica (+1809)",
    "Dominican Republic (+1809)" => "Dominican Republic (+1809)",
    "Ecuador (+593)" => "Ecuador (+593)",
    "Egypt (+20)" => "Egypt (+20)",
    "El Salvador (+503)" => "El Salvador (+503)",
    "Equatorial Guinea (+240)" => "Equatorial Guinea (+240)",
    "Eritrea (+291)" => "Eritrea (+291)",
    "Estonia (+372)" => "Estonia (+372)",
    "Ethiopia (+251)" => "Ethiopia (+251)",
    "Falkland Islands (+500)" => "Falkland Islands (+500)",
    "Faroe Islands (+298)" => "Faroe Islands (+298)",
    "Fiji (+679)" => "Fiji (+679)",
    "Finland (+358)" => "Finland (+358)",
    "France (+33)" => "France (+33)",
    "French Guiana (+594)" => "French Guiana (+594)",
    "French Polynesia (+689)" => "French Polynesia (+689)",
    "Gabon (+241)" => "Gabon (+241)",
    "Gambia (+220)" => "Gambia (+220)",
    "Georgia (+7880)" => "Georgia (+7880)",
    "Germany (+49)" => "Germany (+49)",
    "Ghana (+233)" => "Ghana (+233)",
    "Gibraltar (+350)" => "Gibraltar (+350)",
    "Greece (+30)" => "Greece (+30)",
    "Greenland (+299)" => "Greenland (+299)",
    "Grenada (+1473)" => "Grenada (+1473)",
    "Guadeloupe (+590)" => "Guadeloupe (+590)",
    "Guam (+671)" => "Guam (+671)",
    "Guatemala (+502)" => "Guatemala (+502)",
    "Guinea (+224)" => "Guinea (+224)",
    "Guinea - Bissau (+245)" => "Guinea - Bissau (+245)",
    "Guyana (+592)" => "Guyana (+592)",
    "Haiti (+509)" => "Haiti (+509)",
    "Honduras (+504)" => "Honduras (+504)",
    "Hong Kong (+852)" => "Hong Kong (+852)",
    "Hungary (+36)" => "Hungary (+36)",
    "Iceland (+354)" => "Iceland (+354)",
    "India (+91)" => "India (+91)",
    "Indonesia (+62)" => "Indonesia (+62)",
    "Iran (+98)" => "Iran (+98)",
    "Iraq (+964)" => "Iraq (+964)",
    "Ireland (+353)" => "Ireland (+353)",
    "Israel (+972)" => "Israel (+972)",
    "Italy (+39)" => "Italy (+39)",
    "Jamaica (+1876)" => "Jamaica (+1876)",
    "Japan (+81)" => "Japan (+81)",
    "Jordan (+962)" => "Jordan (+962)",
    "Kazakhstan (+7)" => "Kazakhstan (+7)",
    "Kenya (+254)" => "Kenya (+254)",
    "Kiribati (+686)" => "Kiribati (+686)",
    "Korea North (+850)" => "Korea North (+850)",
    "Korea South (+82)" => "Korea South (+82)",
    "Kuwait (+965)" => "Kuwait (+965)",
    "Kyrgyzstan (+996)" => "Kyrgyzstan (+996)",
    "Laos (+856)" => "Laos (+856)",
    "Latvia (+371)" => "Latvia (+371)",
    "Lebanon (+961)" => "Lebanon (+961)",
    "Lesotho (+266)" => "Lesotho (+266)",
    "Liberia (+231)" => "Liberia (+231)",
    "Libya (+218)" => "Libya (+218)",
    "Liechtenstein (+417)" => "Liechtenstein (+417)",
    "Lithuania (+370)" => "Lithuania (+370)",
    "Luxembourg (+352)" => "Luxembourg (+352)",
    "Macao (+853)" => "Macao (+853)",
    "Macedonia (+389)" => "Macedonia (+389)",
    "Madagascar (+261)" => "Madagascar (+261)",
    "Malawi (+265)" => "Malawi (+265)",
    "Malaysia (+60)" => "Malaysia (+60)",
    "Maldives (+960)" => "Maldives (+960)",
    "Mali (+223)" => "Mali (+223)",
    "Malta (+356)" => "Malta (+356)",
    "Marshall Islands (+692)" => "Marshall Islands (+692)",
    "Martinique (+596)" => "Martinique (+596)",
    "Mauritania (+222)" => "Mauritania (+222)",
    "Mayotte (+269)" => "Mayotte (+269)",
    "Mexico (+52)" => "Mexico (+52)",
    "Micronesia (+691)" => "Micronesia (+691)",
    "Moldova (+373)" => "Moldova (+373)",
    "Monaco (+377)" => "Monaco (+377)",
    "Mongolia (+976)" => "Mongolia (+976)",
    "Montserrat (+1664)" => "Montserrat (+1664)",
    "Morocco (+212)" => "Morocco (+212)",
    "Mozambique (+258)" => "Mozambique (+258)",
    "Myanmar (+95)" => "Myanmar (+95)",
    "Namibia (+264)" => "Namibia (+264)",
    "Nauru (+674)" => "Nauru (+674)",
    "Nepal (+977)" => "Nepal (+977)",
    "Netherlands (+31)" => "Netherlands (+31)",
    "New Caledonia (+687)" => "New Caledonia (+687)",
    "New Zealand (+64)" => "New Zealand (+64)",
    "Nicaragua (+505)" => "Nicaragua (+505)",
    "Niger (+227)" => "Niger (+227)",
    "Nigeria (+234)" => "Nigeria (+234)",
    "Niue (+683)" => "Niue (+683)",
    "Norfolk Islands (+672)" => "Norfolk Islands (+672)",
    "Northern Marianas (+670)" => "Northern Marianas (+670)",
    "Norway (+47)" => "Norway (+47)",
    "Oman (+968)" => "Oman (+968)",
    "Palau (+680)" => "Palau (+680)",
    "Panama (+507)" => "Panama (+507)",
    "Papua New Guinea (+675)" => "Papua New Guinea (+675)",
    "Paraguay (+595)" => "Paraguay (+595)",
    "Peru (+51)" => "Peru (+51)",
    "Philippines (+63)" => "Philippines (+63)",
    "Poland (+48)" => "Poland (+48)",
    "Portugal (+351)" => "Portugal (+351)",
    "Puerto Rico (+1787)" => "Puerto Rico (+1787)",
    "Qatar (+974)" => "Qatar (+974)",
    "Reunion (+262)" => "Reunion (+262)",
    "Romania (+40)" => "Romania (+40)",
    "Russia (+7)" => "Russia (+7)",
    "Rwanda (+250)" => "Rwanda (+250)",
    "San Marino (+378)" => "San Marino (+378)",
    "Sao Tome &amp; Principe (+239)" => "Sao Tome &amp; Principe (+239)",
    "Saudi Arabia (+966)" => "Saudi Arabia (+966)",
    "Senegal (+221)" => "Senegal (+221)",
    "Serbia (+381)" => "Serbia (+381)",
    "Seychelles (+248)" => "Seychelles (+248)",
    "Sierra Leone (+232)" => "Sierra Leone (+232)",
    "Singapore (+65)" => "Singapore (+65)",
    "Slovak Republic (+421)" => "Slovak Republic (+421)",
    "Slovenia (+386)" => "Slovenia (+386)",
    "Solomon Islands (+677)" => "Solomon Islands (+677)",
    "Somalia (+252)" => "Somalia (+252)",
    "South Africa (+27)" => "South Africa (+27)",
    "Spain (+34)" => "Spain (+34)",
    "Sri Lanka (+94)" => "Sri Lanka (+94)",
    "St. Helena (+290)" => "St. Helena (+290)",
    "St. Kitts (+1869)" => "St. Kitts (+1869)",
    "St. Lucia (+1758)" => "St. Lucia (+1758)",
    "Sudan (+249)" => "Sudan (+249)",
    "Suriname (+597)" => "Suriname (+597)",
    "Swaziland (+268)" => "Swaziland (+268)",
    "Sweden (+46)" => "Sweden (+46)",
    "Switzerland (+41)" => "Switzerland (+41)",
    "Syria (+963)" => "Syria (+963)",
    "Taiwan (+886)" => "Taiwan (+886)",
    "Tajikstan (+7)" => "Tajikstan (+7)",
    "Thailand (+66)" => "Thailand (+66)",
    "Togo (+228)" => "Togo (+228)",
    "Tonga (+676)" => "Tonga (+676)",
    "Trinidad &amp; Tobago (+1868)" => "Trinidad &amp; Tobago (+1868)",
    "Tunisia (+216)" => "Tunisia (+216)",
    "Turkey (+90)" => "Turkey (+90)",
    "Turkmenistan (+7)" => "Turkmenistan (+7)",
    "Turkmenistan (+993)" => "Turkmenistan (+993)",
    "Turks &amp; Caicos Islands (+1649)" => "Turks &amp; Caicos Islands (+1649)",
    "Tuvalu (+688)" => "Tuvalu (+688)",
    "Uganda (+256)" => "Uganda (+256)",
    "UK (+44)" => "UK (+44)",
    "Ukraine (+380)" => "Ukraine (+380)",
    "United Arab Emirates (+971)" => "United Arab Emirates (+971)",
    "Uruguay (+598)" => "Uruguay (+598)",
    "USA (+1)" => "USA (+1)",
    "Uzbekistan (+7)" => "Uzbekistan (+7)",
    "Vanuatu (+678)" => "Vanuatu (+678)",
    "Vatican City (+379)" => "Vatican City (+379)",
    "Venezuela (+58)" => "Venezuela (+58)",
    "Vietnam (+84)" => "Vietnam (+84)",
    "Virgin Islands - British (+1284)" => "Virgin Islands - British (+1284)",
    "Virgin Islands - US (+1340)" => "Virgin Islands - US (+1340)",
    "Wallis &amp; Futuna (+681)" => "Wallis &amp; Futuna (+681)",
    "Yemen (North)(+969)" => "Yemen (North)(+969)",
    "Yemen (South)(+967)" => "Yemen (South)(+967)",
    "Zambia (+260)" => "Zambia (+260)",
    "Zimbabwe (+263)" => "Zimbabwe (+263)",


    "used_vehicles" => 'Used vehicles for sale',
    "show_used_vehicles" => "Show Used Vehicles",
    "show_used_vehicle" => "Show Used Vehicle",
    "edit_used_vehicle" => "Edit Used Vehicle",
    "new_used_vehicle" => "Create Used Vehicle",
    "vehicle_name" => "Vehicle Name",
    "front_side_image" => "Front Side Image",
    "back_side_image" => "Back Side Image",
    "right_side_image" => "Right Side Image",
    "left_side_image" => "Left Side Image",
    "inside_vehicle_image" => "Inside Vehicle Image",
    "vehicle_dashboard_image" => "Vehicle Dashboard Image",
    "traffic_pdf" => "Traffic Report File",
    "link" => "Link",
    "ad_for" => "Ad for",
    'pending' => 'Pending',
    'approved' => 'Approved',
    'rejected' => 'Rejected',
    'replied' => 'Replied',
    'start_date' => 'Start Date',
    'end_date' => 'End Date',
    "ad_types" => "Ad Types",
    "show_ad_types" => "Show Ad Types",
    "show_ad_type" => "Show Ad Type",
    "edit_ad_type" => "Edit Ad Type",
    "org_roles" => "Roles",
    "general_data" => "General Data",

    "edit_vehicle" => "Edit Vehicle",
    "3d_message" => "3D file extension must be '.glb'",

    "show_available_payment_methods" => "Show Available Payment Methods",
    'work_times' => "Show Work Times",
    'show_work_times' => "Work Times",
    'edit_work_times' => "Edit Work Time",

    'days_off' => "Days Off",
    'show_days_off' => "Show Days Off",
    'show_day_off' => "Show Day Off",
    'edit_day_off' => "Edit Day Off",
    'new_day_off' => "Create Day Off",

    "phones" => "Phones",
    "show_phones" => "Show Phones",
    "show_phone" => "Show Phone",
    "edit_phone" => "edit Phone",
    "create_phone" => "Create Phone",

    "show_reviews" => "Show Reviews",
    "show_review" => "Show Review",
    "edit_review" => "Edit Review",
    "review" => "Review",
    "total_rates" => "Total Rates",

    "priority" => "Priority",

    'accessories' => 'Accessories',
    'carWashServices' => 'Services',
    'packages' => 'Packages',
    'brokerPackages' => 'Packages',
    'miningCenterService' => 'Services',
    'rental_office_cars' => 'Rental Office Cars',
    'inspectionCenterService' => 'Services',
    'tireExchangeService' => 'Services',
    'trafficServices' => 'Services',

    "read-org" => "Read Mowater Card Organizations",
    "delete-org" => "Delete Mowater Card Organizations",
    "update-org" => "Delete Mowater Card Organizations",

    "subscribed" => "Subscribed",
    "unsubscribed" => "Unsubscribed",
    "subscription" => "Subscription",

    "available_payment_methods" => "Available Payment Methods",

    "read-branch" => "Read Branches",
    "update-branch" => "Update Branches",
    "create-branch" => "Create Branches",
    'delete-branch' => 'Delete Branches',

    "show_branches" => "Show Branches",
    "show_branch" => "Show Branch",
    "edit_branch" => "Edit Branch",
    "new_branch" => "Create Branch",

    "address_en" => "Address In English",
    "address_ar" => "Address In Arabic",

    "read-org-branch" => "Read Organization's Branches",
    "update-org-branch" => "Update Organization's Branches",
    "create-org-branch" => "Create Organization's Branches",
    "delete-org-branch" => "Delete Organization's Branches",

    "org_ads" => "Ads",
    "org_discount_cards" => "Mowater Cards",
    "read-offers" => "Read Offers",
    "update-offers" => "Update Offers",
    "create-offers" => "Create Offers",
    "day_offs" => "Days Off",
    "work_time" => "Work Times",
    "contact" => "Contacts",

    "show_branches_users" => "Show Branches Users",
    "show_branch_user" => "Show Branch User",
    "edit_branch_user" => "Edit Branch User",
    "new_branch_user" => "Create Branch User",

    "show_available_vehicles" => "Show Available Vehicles",
    "edit_available_vehicles" => "Edit Available Vehicle",
    "show_available_products" => "Show Available Products",
    "edit_available_products" => "Edit Available Products",
    "show_available_services" => "Show Available Services",
    "edit_available_services" => "Edit Available Services",

    "category" => "Category",
    "manufacturing_year" => "Manufacturing Year",

    "available_vehicles" => "Available Vehicles",
    "available_products" => "Available Products",
    "available_services" => "Available Services",

    "reserve_vehicles" => "Vehicle Reservations",

    "show_vehicles_reservations" => "Show Vehicles Reservations",
    "show_vehicle_reservation" => "Show Vehicle Reservation",
    "edit_vehicle_reservation" => "Edit Vehicle Reservation",
    "action_by" => "Action By",
    "show_vehicle_details" => "Show Vehicle Details",
    "show_service_details" => "Show Service Details",
    "show_product_details" => "Show Product Details",
    "vehicle_details" => "Vehicle Details",

    "id_number" => "ID Number",
    "id_type" => "ID Type",
    "passport" => "Passport",
    "national_id" => "National ID",
    "personal_ID" => "ID Image",
    "driving_license" => "Driving License Image",
    "is_mowater" => "Using Mowater Card",
    'tests' => 'Driving test reservations',
    "show_tests_reservations" => "Show Tests Reservations",
    "show_test_reservation" => "Show Test Reservation",

    "credit_card_number" => "Credit Card Number",
    "test_details" => "Test Drive Reservation Details",

    "daily_rental_price" => "Daily Rental Price",
    "weekly_rental_price" => "Weekly Rental Price",
    "monthly_rental_price" => "Monthly Rental Price",
    "yearly_rental_price" => "Yearly Rental Price",
    "color" => "Color",
    "add_more" => "Add More",

    "cars_properties" => "Cars Properties",
    "show_cars_properties" => "Show Cars Properties",
    "show_property" => "Show Property",
    "edit_property" => "Edit Property",
    "new_property" => "Create Property",

    "rental_laws" => "Rental Laws",
    "show_rental_laws" => "Show Rental Laws",
    "show_rental_law" => "Show Rental Law",
    "edit_rental_law" => "Edit Rental Law",
    "create_rental_law" => "Create Rental Law",
];


