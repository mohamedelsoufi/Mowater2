...
class CarWashService extends Model
{
    ...
    protected $fillable = ['id', 'car_wash_id', 'name_en', 'name_ar', 'description_en'
    ...
    {
        return $this->belongsToMany(RequestCarWash::class, 'car_wash_request_services');
    }

    public function car_wash_service_available_reservation($date, $id, $module, $service_model, $service_id)
    {
        try {
            ...
            if ($find_day !== false) {
                ...
                while ($new_time < $to) {
                    ...
                    $reservations = $service->carWashRequests;
                    if (!$reservations == null) {
                        foreach ($reservations as $key => $reservation) {
                            if ($reservation->pivot->car_wash_service_id == $service_id) {
