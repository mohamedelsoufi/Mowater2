...
class GetInsuranceCompaniesResource extends JsonResource
{
    public function toArray($request)
    {
        ...
        if ($request->routeIs('insurance.show'))
        ...
        if ($request->routeIs('insurance.show'))
            $data['area'] = $this->area;
        if ($request->routeIs('insurance.show')) {
            $data['packages'] = InsuranceCompanyPackagesResource::collection($this->packages);
