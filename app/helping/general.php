...
function productMowaterCard($products)
{
...
function serviceMowaterCard($services)
{
...
function vehicleMowaterCard($vehicles)
{
...
function productOffers($products)
{
    foreach ($products as $product) {
        $product->kind = 'product';
        foreach ($product->offers as $offer) {
...
function serviceOffers($services)
{
    foreach ($services as $service) {
        $service->kind = 'service';
        foreach ($service->offers as $offer) {
...
function vehicleOffers($vehicles)
{
    foreach ($vehicles as $vehicle) {
        $vehicle->kind = 'vehicle';
        foreach ($vehicle->offers as $offer) {
...
function GeneralMowaterCard($items)
{
    foreach ($items as $item) {
        foreach ($item->offers as $offer) {
            ...
            $percentage_value = ((100 - $offer->discount_value) / 100);
            if ($discount_type == 'percentage') {
                $price_after_discount = $item->price * $percentage_value;
                $item->card_discount_value = $offer->discount_value . '%';
                $item->card_price_after_discount = $price_after_discount . ' BHD';
                $item->card_number_of_uses_times = $offer->number_of_uses_times == 'endless' ? __('words.endless') : $offer->specific_number;
            } else {
                $price_after_discount = $item->price - $offer->discount_value;
                $item->card_discount_value = $offer->discount_value . ' BHD';
                $item->card_price_after_discount = $price_after_discount . ' BHD';
                $item->card_number_of_uses_times = $offer->number_of_uses_times == 'endless' ? __('words.endless') : $offer->specific_number;
            }
            $item->notes = $offer->notes;
            $item->makeHidden('offers');
        }
    }
...
function rentalVehicleMowaterCard($vehicles)
{
    foreach ($vehicles as $vehicle) {
        foreach ($vehicle->offers as $offer) {
            $discount_type = $offer->discount_type;
            ...
            if ($discount_type == 'percentage') {
                ...
                $daily_price_after_discount = $vehicle->daily_rental_price * $percentage_value;
                $vehicle->card_daily_price_after_discount = $daily_price_after_discount . ' BHD';

                $weekly_price_after_discount = $vehicle->weekly_rental_price * $percentage_value;
                $vehicle->card_weekly_price_after_discount = $weekly_price_after_discount . ' BHD';

                $monthly_price_after_discount = $vehicle->monthly_rental_price * $percentage_value;
                $vehicle->card_monthly_price_after_discount = $monthly_price_after_discount . ' BHD';

                $yearly_price_after_discount = $vehicle->yearly_rental_price * $percentage_value;
                $vehicle->card_yearly_price_after_discount = $yearly_price_after_discount . ' BHD';

                $vehicle->card_number_of_uses_times = $offer->number_of_uses_times == 'endless' ? __('words.endless') : $offer->specific_number;

            } else {
                $daily_price_after_discount = $vehicle->daily_rental_price - $offer->discount_value;
                $weekly_price_after_discount = $vehicle->weekly_rental_price - $offer->discount_value;
                $monthly_price_after_discount = $vehicle->monthly_rental_price - $offer->discount_value;
                $yearly_price_after_discount = $vehicle->yearly_rental_price - $offer->discount_value;
...
function rentalVehicleOffers($vehicles)
{
    foreach ($vehicles as $vehicle) {
        foreach ($vehicle->offers as $offer) {
            $discount_type = $offer->discount_type;
